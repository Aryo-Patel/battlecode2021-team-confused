...
public strictfp class SomeRefractoring{
    ...
    static MapLocation decodeLocation(int encodedLocation) throws GameActionException {
        ...
        for (Pair<Integer, Integer> dxdy: new Pair[]{new Pair<>(128, 0), new Pair<>(0, -128), new Pair<>(0, 128),
                new Pair<>(-128, 0)}){
            MapLocation alternative = actualLocation.translate(dxdy.getKey(), dxdy.getValue());
    ...
    static Pair<MapLocation, Integer> findMinBuilding(HashMap<MapLocation, Integer> queueEC){
        ...
        return new Pair<>(minLocation, minConviction);
    }
    ...
    static void runEnlightenmentCenter() throws GameActionException {
        ...
        // updates hash maps of ECs based on flags of spawned bots TODO: this doesn't only check for ECs right?
        for (int robot : spawnedRobots) {
            if (rc.canGetFlag(robot)) {
                ...
                if (team == ownEC) {
                    enemyECs.remove(robotLoc); // will return null if loc not present in enemyECs
        ...
        if (mod8Turn == 7 || mod8Turn == 0) {
            ...
            for (HashMap<MapLocation, Integer> queueEC: new HashMap[]{neutralECs, enemyECs}){
                Pair<MapLocation, Integer> ECInfo = findMinBuilding(queueEC);
            ...
            boolean setFlagToTargetEC = false;
            Pair<MapLocation, Integer> ECInfo = findMinBuilding(enemyECs);
            if (ECInfo.getKey() != null){
            ...
            spawnRobots(RobotType.MUCKRAKER, 3);
        }
    ...
    static void runSlanderer() throws GameActionException {
        ...
        for (RobotInfo robot : nearbyRobots) {
            ...
            if (robot.getTeam() == enemy) {
                ...
                else if (shortestDist < nearestMuckrakerDistance && robot.getType() == RobotType.MUCKRAKER )
                    nearestEnemyMuckraker = robot; nearestMuckrakerDistance = shortestDist;
            }
        }
